package TP6E1;

/*
 * @author Grupo 11
 */
public class Main {

    public static void main(String[] args) {

        /*
        Productos p11 = new Productos ("electronica", "Heladera" , 750);
        Productos p12 = new Productos ("electronica", "Lavarropas" , 999);
        Productos p13 = new Productos ("electronica", "Televesor" , 500);
        
        Productos p22 = new Productos ("perfumeria", "PacoRabanne" , 1500);
        Productos p21 = new Productos ("perfumeria", "CH" , 2000);
        Productos p23 = new Productos ("perfumeria", "CK" , 1000);
        
        Productos p31 = new Productos ("ropa", "Remera" , 99);
        Productos p32 = new Productos ("ropa", "Pantalon" , 150);
        Productos p33 = new Productos ("ropa", "Medias" , 10);
         */
    }

}
